...
<head>
    ...
    <title>云平台客户端V1-包装物拨入查询</title>
    <meta name="description" content="云平台客户端V1-包装物拨入查询">
    ...
    <div class="am-scrollable-horizontal" style="margin-top: 15px;" id="tabmx">
        <table class="am-table am-table-bordered am-table-striped am-text-nowrap am-table-centered" id="hztable">
            ...
            <tr>
                ...
                <th class="am-text-middle">拨出门店名称</th>
...
<script type="text/javascript">
    ...
    function loadInfo(cxtj) {
        ...
        if(cxfs=='0'){
            $.ajax({
                ...
                type: "post",
                async: false,
                data: {cxtj: cxtj, f_ksrq: ksrq,f_jsrq: jsrq, timeer: new Date()},
                success: function (data) {
                    var spgjList = JSON.parse(data);
                    if (spgjList.length > 0) {
                        for (var i = 0; i < spgjList.length; i++) {
                            var spgj = spgjList[i];
                            salehtml += "<tr>\n<td class=\"am-text-middle\"><a href=\"javascript:showDetail('" + spgj.F_DJH + "','"+ spgj.F_BMBM +"')\" >详情</a> </td>\n" +
                                "<td class=\"am-text-left\">" + spgj.F_ZDRQ + "</td>\n" +
                                "<td class=\"am-text-left\">" + spgj.F_BMMC + "</td>\n" +
                    ...
                    if (spgjList.length > 0) {
                        for (var i = 0; i < spgjList.length; i++) {
                            ...
                            salehtml += "<tr>\n<td class=\"am-text-middle\">" + spgj.F_DJH + "</td>\n" +
                                ...
                                " <td class=\"am-text-right\">" + -1 * spgj.F_GJDJ + "</td>\n" +
                                " <td class=\"am-text-right\">"+ -1 * spgj.F_GJSL+"</td>\n" +
                                " <td class=\"am-text-right\">"+ -1 * spgj.F_GJJE+"</td>\n</tr>";
                        }
                        $spgjtable.html(salehtml);
                    }
                }, error: function (XMLHttpRequest, textStatus, errorThrown) {
    ...
    //显示单据详情
    function showDetail(f_djh,f_bmbm){
        $('#xsDetailtable').html('');
        $('#xsDetail').modal({closeViaDimmer: false,width:680,height:500});
        $.ajax({
            url: "/packing/queryDjxq",
            type: "post",
            async: false,
            data: {f_djh:f_djh,f_bmbm:f_bmbm, timeer: new Date() },
            success: function (data) {
                ...
                if(dataJson.length>0) {
                    ...
                    for(var i=0;i<dataJson.length;i++){
                        ...
                        var rowhtml="<tr sptm='"+spjson.F_SPTM+"'>"
                            ...
                            +"<td class=\"am-text-right\">"+ -1 * spjson.F_GJDJ +"</td>"
                            +"<td class=\"am-text-right\">"+ -1 * spjson.F_GJSL +"</td>"
                            +"<td class=\"am-text-right\">"+ -1 * spjson.F_GJJE +"</td>"
