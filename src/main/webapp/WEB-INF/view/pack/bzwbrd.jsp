...
<head>
    ...
    <title>云平台客户端V1-包装物拨入单</title>
    <meta name="description" content="云平台客户端V1-包装物拨入单">
...
<div class="am-g">
    <div class="am-u-sm-12 am-u-md-12" id="xsdiv">
        ...
        <div class="am-container am-">
            <div class="am-u-sm-7 am-u-md-7">拨入仓库编码：<select data-am-selected id="brckbm"></select></div>
            <div class="am-u-sm-7 am-u-md-7">拨出网点编码：<select data-am-selected id="bcwdbm"></select></div>
...
<script type="text/javascript">
    ...
    function addOption() {
        //拨入仓库(900001)
        var brckbm = "<%=f_brcdmrck %>";
        $.ajax({
            url: "/packing/getBrckmc",
            type: "post",
            async: false,
            data: {brckbm:brckbm, timeer: new Date() },
            success: function (data) {
                var brckmc = data;
                var brckHtml = "<option value='"+brckbm+"'>"+brckmc+"</option>";
                $("#brckbm").html(brckHtml);
            },
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                for (var i = 0; i < dataJson.length; i++){
                    var bcwdxx = dataJson[i];
                    bcwdHtml += "<option value='"+bcwdxx.F_BMBM+"'>"+bcwdxx.F_BMMC+"</option>";
                }
                $("#bcwdbm").html(bcwdHtml);
            },
            error: function (XMLHttpRequest, textStatus, errorThrown) {
                var sessionstatus = XMLHttpRequest.getResponseHeader("sessionstatus");
                if (sessionstatus == "logintimeout") {
    ...
    //保存单据
    function savebill(){
        var brckbm=$('#brckbm').val();
        var bcwdbm=$('#bcwdbm').val();
        if(brckbm===undefined|| brckbm.length<0){
            alertMsg('请选择拨入仓库！');
            return;
        }
        if(bcwdbm===undefined|| bcwdbm.length<0){
            alertMsg('请选择网点！');
            return;
        }
        ...
        setTimeout(function(){
            $.ajax({
                url: "/packing/bzwbrdBill",
                type: "post",
                async: false,
                data: {brckbm:brckbm,bcwdbm:bcwdbm,spxx:spxx,timeer: new Date() },
                success: function (data) {
                    if(data=="ok"){
                        alertMsg("保存成功！");
                        clearpage();
    ...
    function searchSpda(){
        ...
        $.ajax({
            ...
            type: "post",
            async: false,
            data: {spxx:spxx,f_bcwdbm:$('#brckbm').val(), timeer: new Date() },
            success: function (data) {
    ...
    function clearpage(){
        $('#bcwdbm').val('');
        var bcwdHtml = "";
        for (var i = 0; i < bcwdbmJson.length; i++){
            var bcwdxx = bcwdbmJson[i];
            bcwdHtml += "<option value='"+bcwdxx.F_BMBM+"'>"+bcwdxx.F_BMMC+"</option>";
        }
        $("#bcwdbm").html(bcwdHtml);
        var tshtml="<tr id=\"tishitr\">\n" +
