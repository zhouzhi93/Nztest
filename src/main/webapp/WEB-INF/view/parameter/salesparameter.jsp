...
<head>
    ...
    <title>云平台客户端V1-销售台账</title>
    <meta name="description" content="云平台客户端V1-销售台账">
            ...
            <div class="am-container">
                <div class="am-u-sm-4 am-u-md-4" style="padding-left: 0px;">
                    ...
                    <div class="am-u-sm-4 am-u-md-4" style="padding: 0px;">
                        <input id="xstzKsrq" type="text" class="am-radius am-form-field am-input-sm" placeholder="开始日期" data-am-datepicker readonly required />
                    </div>
                    <div class="am-u-sm-1 am-u-md-1" style="padding: 0px;text-align: center;">
                        至
                    </div>
                    <div class="am-u-sm-4 am-u-md-4" style="padding: 0px;">
                        <input id="xstzJsrq" type="text" class="am-radius am-form-field am-input-sm" placeholder="结束日期" data-am-datepicker readonly required />
                    </div>
                </div>
...
<%--                    <div class="am-u-sm-6 am-u-md-6 xsqxdix" style="margin-top: 10px;">
            ...
            <div class="am-container am-scrollable-horizontal" style="margin-top: 20px;" id="hovertables">
                <table class="am-table am-table-bordered am-table-centered am-table-striped am-text-nowrap am-table-compact" >
                    <thead>
                    <tr>
                        ...
                        <th class="am-text-middle xsqxdix">身份证号</th>
...
<script type="text/javascript">
    $(function(){
        ...
        $('#xstzKsrq').datepicker('setValue','<%=ksrq%>');
        $('#xstzJsrq').datepicker('setValue','<%=newdata%>');
    ...
    function loadInfo(pageIndex,pageSize){
        var ksrq=$('#xstzKsrq').val();
        var jsrq=$('#xstzJsrq').val();
        var spmc=$('#xstzSpmc').val();
        var sptm=$('#xstzSptm').val();
        var djh =$('#xstzDjh').val();
        ...
        $.ajax({
            ...
            type: "post",
            async: false,
            data: {ksrq:ksrq,jsrq:jsrq,spmc:spmc,sptm:sptm,djh:djh,khbm:khbm,khmc:khmc,qybm:"",
            ...
            success: function (data) {
                ...
                if(spdalist.length>0) {
                    for(var i=0;i<spdalist.length;i++){
                        ...
                        rowhtml+="<td class=\"am-text-middle\" title='"+spjson.F_KHMC+"'>"+spjson.F_KHMC+"</td>"
                        rowhtml+="<td class=\"am-text-middle\" title='"+spjson.F_DH+"'>"+spjson.F_DH+"</td>"
                        if('<%=ypd%>' == '1' || "<%=ypd%>" == '0'){
                            rowhtml+="<td class=\"am-text-middle\" title='"+spjson.F_SFZH+"'>"+spjson.F_SFZH+"</td>"
                        }
                        rowhtml+="<td class=\"am-text-middle\" title='"+spjson.F_JLDW+"'>"+spjson.F_JLDW+"</td>"
                        rowhtml+="<td class=\"am-text-middle\" title='"+spjson.F_XSSL+"'>"+spjson.F_XSSL+"</td>"
                        rowhtml+="<td class=\"am-text-middle\" title='"+spjson.F_XSDJ+"'>"+spjson.F_XSDJ+"</td>"
                        rowhtml+="<td class=\"am-text-middle\" title='"+spjson.F_SSJE+"'>"+spjson.F_SSJE+"</td>"
                        rowhtml+="</tr>";
                        var colspanval=8;
                        if('<%=ypd%>' == '1' || "<%=ypd%>" == '0'){
                            colspanval=10
                        }
                        zxl+=eval(spjson.F_XSSL);
                        zje+=eval(spjson.F_SSJE);
    ...
    function getexcel(){
        ...
        var khbm=$('#xstzKhbm').val();
        var khmc=$('#xstzKhmc').val();
        toexcel(ksrq,jsrq,spmc,sptm,djh,khbm,khmc);
    }
    function toexcel(ksrq,jsrq,spmc,sptm,djh,khbm,khmc) {
        window.location.href = "/excel/getSalesParameterExcel?ksrq="+ksrq+"&jsrq="+jsrq+"&spmc="+spmc+"&sptm="+sptm+"&djh="+djh+"&khbm="+khbm+"&khmc="+khmc+"&qybm=";
    }
